export async function onRequestPost(context) {
    ...
    try {
        ...
        // SECURE VERIFICATION (Padding-Safe Base64)
        const tokenParts = token.split('.');
        if (tokenParts.length !== 3) return new Response("Invalid token format", { status: 401 });
            
        let base64 = tokenParts[1].replace(/-/g, '+').replace(/_/g, '/');
        // FIX: Add missing padding so atob() doesn't crash
